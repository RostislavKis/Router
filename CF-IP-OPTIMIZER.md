...
Файл: `/etc/cf-optimizer/99-cf-dpi-bypass.nft`

> **Важно:** файл НЕ кладётся в `/etc/nftables.d/` — fw4 включает эту директорию
> внутри контекста таблицы `inet fw4`, и отдельное объявление таблицы там вызывает
> синтаксическую ошибку → fw4 не поднимается → нет NAT.
> Файл применяется через `setup-cf-optimizer.sh` (init.d/cf-optimizer при старте).
...
При перезагрузке применяется через `init.d/cf-optimizer` (после старта Mihomo).
...
patches/
    cf-ip-update.sh       Блок 1: обновление CF edge IP
                          ⚠ НЕ в репозитории (gitignored) — персональный скрипт
                            для прокси за Cloudflare CDN. При установке через
                            install.sh пропускается ([skip]); setup-cf-optimizer.sh
                            создаёт заглушку-заменитель автоматически.
    sni-scan.sh           Блок 2: сканирование SNI
    99-cf-dpi-bypass.nft  Блок 3: DPI bypass через nftables MSS
                          Устанавливается в /etc/cf-optimizer/ (не /etc/nftables.d/)
