...
### Быстрый способ: один скрипт с ПК (`deploy.py`)

Скопируй `deploy.py` из репозитория, впиши свои данные в верхнюю секцию и запусти:
...
# Запустить деплой
python -X utf8 deploy.py
```

`deploy.py` делает всё автоматически:

1. Подключается к роутеру по SSH
2. Загружает все файлы из `patches/` через SFTP (CRLF→LF нормализация)
3. Запускает `setup-cf-optimizer.sh` с выводом в реальном времени
4. Убирает временные файлы с роутера

> `deploy.py` содержит учётные данные — он находится в `.gitignore` и не попадает в репозиторий. Создай его локально из шаблона ниже или возьми из рабочей копии.
...
### Ручной способ (через SSH вручную)
